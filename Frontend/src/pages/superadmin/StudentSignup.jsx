...
export default function StudentSignup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { name, email, password, confirmPassword, department, year, college_name } = formData;
    ...
    try {
      const response = await fetch("http://localhost:8000/api/student-signup/", {
        ...
        body: JSON.stringify({ name, email, password, department, year, college_name }),
      });
      ...
      if (response.ok) {
        ...
        }, 2000); // Delay to allow the toast message to display
      } else {
    ...
    <div className="w-screen h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="w-3/4 min-h-3/4 max-h-[85%] bg-white shadow-lg rounded-lg flex items-stretch p-2">
        ...
        <div className="flex-1 p-6 flex flex-col items-center justify-evenly">
          <p className="text-3xl">Student Signup</p>
          ...
          <form onSubmit={handleSubmit} className="w-3/4 flex flex-col pt-4 items-center">
            <div className="space-y-4 mb-6 w-full relative">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={handleChange}
                  className="w-full p-3 rounded-md border border-gray-300 focus:outline-none focus:ring-yellow-400"
                  required
                />
              </div>
            ...
            {/* Sign Up Button */}
            <button type="submit" className="p-3 rounded-2xl w-full font-semibold bg-[#FECC00]">
              Sign Up
            </button>
          </form>
        </div>
