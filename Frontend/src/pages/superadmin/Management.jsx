...
//         <div className="flex items-center space-x-1 ml-157">
...
export default function MailPage() {
    ...
    const fetchAutoApproval = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/api/get-auto-approval-status/",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleSelectAll = (type) => {
    ...
    if (type === "job") {
      setSelectedJobs((prev) => {
        const newSelected =
          prev.length === jobs.length ? [] : jobs.map((job) => job._id);
        console.log(`Selected jobs: ${newSelected}`);
        return newSelected;
      });
    } else if (type === "achievement") {
      setSelectedAchievements((prev) => {
        const newSelected =
          prev.length === achievements.length
            ? []
            : achievements.map((achievement) => achievement._id);
        console.log(`Selected achievements: ${newSelected}`);
        return newSelected;
      });
    } else {
      setSelectedInternships((prev) => {
        const newSelected =
          prev.length === internships.length
            ? []
  ...
  const handleBulkDelete = async (type) => {
    ...
    if (
      window.confirm(`Are you sure you want to delete all selected ${type}s?`)
    ) {
      try {
        const promises = ids.map((id) => handleDelete(id, type));
        await Promise.all(promises);
  ...
  return (
    <div className="container mx-auto p-4 ml-60">
      <SuperAdminPageNavbar />
      <h1 className="text-2xl font-semibold mb-4 text-gray-800">Manage Jobs</h1>

      <div className="mb-4 flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-4">
        {/* Navigation Buttons */}
        <button
          className={`px-4 py-2 rounded ${
            visibleSection === "jobs" ? "bg-blue-500 text-white" : "bg-gray-200"
          }`}
          onClick={() => setVisibleSection("jobs")}
        >
          Jobs
