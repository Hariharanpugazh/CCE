...
export default function AchievementDashboard() {
  ...
  useEffect(() => {
    ...
    if (selectedCompany) {
      filtered = filtered.filter((achievement) =>
        achievement.company_name.toLowerCase().includes(selectedCompany.toLowerCase()),
      )
    }
    if (searchQuery) {
      filtered = filtered.filter((achievement) => achievement.name.toLowerCase().includes(searchQuery.toLowerCase()))
    }
  ...
  const recentAchievements = useMemo(() => {
    ...
    if (carouselFilter) {
      filtered = filtered.filter((achievement) => achievement.achievement_type === carouselFilter)
    }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {currentAchievements.map((achievement) => (
            ...
            >
              ...
              {/* Name and Role */}
              <h2 className="text-lg md:text-xl font-semibold text-gray-800 mb-1">{achievement.name}</h2>
              <p className="text-xs md:text-sm text-gray-500 mb-2">
                {achievement.achievement_type}
                <br />
                <span className="text-amber-400 text-xs md:text-sm font-medium">{achievement.batch}</span>
              </p>

              {/* Email */}
              <p className="text-xs md:text-sm text-gray-600 mb-4">{achievement.email}</p>
          ...
          <div className="container p-4 md:p-10">
            <div className="flex flex-col md:flex-row">
              <div className="w-full md:w-2/5 mb-8 md:mb-0">
                <h3 className="text-2xl md:text-3xl mb-5">Centre for Competitive Exams</h3>
                <div className="w-full md:w-3/4">
                  <p className="text-xs md:text-sm">
                    CCE focuses on constantly endeavor to identify the potential opportunities for our students to
                    elevate their personality and professional competence, which in turn will enhance their
              ...
              <div className="w-full md:w-3/5 flex flex-wrap justify-between md:pl-20">
                {["Products", "Resources", "Company", "Support"].map((category) => (
            ...
            <div className="my-6 md:my-10 space-y-5">
              ...
              <p className="text-xs md:text-sm">
                &copy; {new Date().getFullYear()} SNS iHub Workplace. All Rights Reserved
              </p>
            </div>
          </div>
        </footer>
