...
export default function ApplicationCard({ application }) {
    ...
    return <div className="flex flex-col p-3 border border-gray-200 rounded-lg justify-between">
        ...
        <div className="flex justify-between items-start">
            <div className="flex flex-col">
                ...
                <div className="flex items-center space-x-3 text-sm">
                    <p className="text-[#8C8C8C] flex items-center"> <i class="bi bi-building w-[15px] mr-[5px]"></i> {application.company_name} </p>
                    <FiCircle style={{ width: "4px", height: "4px", backgroundColor: "#8C8C8C", borderRadius: "50%" }} />
                    <p className="text-[#8C8C8C] flex items-center"> <FiMapPin style={{ width: "15px", marginRight: "5px" }} /> {application.job_location} </p>
                </div>
            </div>
        ...
        <div className="w-[85%] flex flex-wrap space-x-3">
            {
                typeof (application.skills_required ?? application.required_skills) === "string" ?
                    ...
                    :
                    (application.skills_required ?? application.required_skills).map((skill, key) => <p className="p-1 bg-gray-100 text-xs rounded px-2"></p>)
            }
        </div>
        ...
        <div className="flex text-[#8C8C8C] items-center space-x-2 mt-2">
            ...
            <FiCircle style={{ width: "4px", height: "4px", backgroundColor: "#8C8C8C", borderRadius: "50%" }} />
            <p className="underline text-xs truncate w-[65%] leading-none cursor-pointer" onClick={() => window.location.href = application.company_website}> {application.company_website} </p>
        </div>
