    ...
    if request.method == "POST":
        try:
            ...
            if admin_collection.find_one({"email": email}):
                return JsonResponse(
                    {"error": "Admin user with this email already exists"}, status=400
                )
            ...
            admin_user = {"name": name, "email": email, "password": password}

            # Insert the document into the collection
            admin_collection.insert_one(admin_user)

            return JsonResponse(
                {"message": "Admin user created successfully"}, status=200
            )
    ...
    if request.method == "POST":
        try:
            ...
            if admin_user and check_password(password, admin_user["password"]):
                ...
                return JsonResponse(
                    {"message": "Login successful", "tokens": tokens}, status=200
                )
            else:
...
def super_admin_signup(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"message": "Super admin user created successfully"}, status=201
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
    else:
...
def super_admin_login(request):
    if request.method == "POST":
        try:
            ...
            super_admin_user = admin_collection.find_one(
                {"email": email, "role": "super_admin"}
            )
            super_admin_id = super_admin_user.get("_id")

            if super_admin_user and check_password(
...
def job_post(request):
    ...
    if not token:
        return Response(
            {"error": "JWT cookie not found"}, status=status.HTTP_401_UNAUTHORIZED
        )
...
def post_achievement(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"message": "Achievement posted successfully."}, status=201
            )
